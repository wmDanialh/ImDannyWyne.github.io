...
async function initViewCounter() {
  ...
  try {
    ...
    const res = await fetch(`https://hits.sh/dannywyne.github.io/portfolio.svg?t=${timestamp}`, {
      cache: "no-store"
    });
    const text = await res.text();
    const match = text.match(/<title>hits: (\d+)<\/title>/);
    if (match && match[1]) {
      counterEl.textContent = match[1].padStart(6, "0");
    }
  } catch (err) {
